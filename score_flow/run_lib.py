...
def train(config, workdir):
  ...
  for step in range(initial_step, num_train_steps + 1, config.training.n_jitted_steps):
    ...
    if jax.host_id() == 0 and step % config.training.log_freq == 0:
      logging.info("step: %d, training_loss: %.5e" % (step, loss.mean()))
    ...
    if step % config.training.eval_freq == 0:
      ...
      if jax.host_id() == 0:
        logging.info("step: %d, eval_loss: %.5e" % (step, eval_loss.mean()))
...
def evaluate(config,
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_bpd:
      ...
      score_mean_dict = {} # initializing dict to record score_mean
      for repeat in range(begin_repeat_id, bpd_num_repeats):
        ...
        for batch_id in range(begin_batch_id, len(ds_bpd)):
          ...
          # whether use score_mean
          if not use_score_mean:
            bpd = likelihood_fn(step_rng, pstate, data)[0]
          else:
            bpd, score_mean_dict = likelihood_fn(step_rng, pstate, data, 
                                                   score_mean_dict=score_mean_dict,
                                                   train_ds=train_ds_bpd,
                                                   scaler=scaler,
                                                   inverse_scaler=inverse_scaler)[0:2]
          if config.eval.dequantizer:
            bpd = bpd + bpd_d
          bpd = bpd.reshape(-1)
          bpds.extend(bpd)
