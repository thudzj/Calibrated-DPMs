...
def get_likelihood_bound_fn(sde, model, inverse_scaler, hutchinson_type='Rademacher',
                            ...
                            eps_offset=True, use_score_mean=False):
  ...
  def likelihood_bound_fn(prng, state, data, score_mean_dict=None):
  ...
  def likelihood_bound_fn_use_score_mean(prng, state, data, score_mean_dict=None):
    """Compute an unbiased estimate to the log-likelihood in bits/dim.
  ...
  return jax.pmap(likelihood_bound_fn, axis_name='batch') if not use_score_mean else jax.pmap(likelihood_bound_fn_use_score_mean, axis_name='batch')
