...
class OurModelWrapper:
    def __init__(self, _model_fn, noise_schedule, config, which_for_score_mean='train', subsample=None, n_estimates=1):
        ...
        if which_for_score_mean in ['train', 'test']:
            # Build data iterators
            config.data.random_flip = False
            train_ds, eval_ds, _ = datasets.get_dataset(config,
                                                        uniform_dequantization=False,
                                                        evaluation=True)
            self.data_ds = train_ds if which_for_score_mean == 'train' else eval_ds
            self.data_iter = iter(self.data_ds)
    ...
    def estimate_score_mean(self, t):
        score_sum = None
        n_data = 0
        print("estimate score mean for time", t, self.noise_schedule.marginal_log_mean_coeff(t/999.).exp(), self.noise_schedule.marginal_std(t/999.))
        while 1:
            try:
                ...
                self.data_iter = iter(self.data_ds)
                break
            batch = torch.from_numpy(batch['image']._numpy()).to(t.device).float()
            batch = batch.permute(0, 3, 1, 2)
            x = self.scaler(batch)

            vec_t = torch.ones(x.shape[0], device=x.device) * t
            mean, std = self.noise_schedule.marginal_log_mean_coeff(vec_t/999.).exp(), self.noise_schedule.marginal_std(vec_t/999.)
            ...
            n_data += self.n_estimates * x.shape[0]
        
        # assert n_data == self.subsample * self.n_estimates, (n_data, self.subsample, self.n_estimates)
